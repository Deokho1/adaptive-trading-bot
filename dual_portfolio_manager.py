...
Manages BTC and ETH as independent engines with adaptive weighting
"""
...
class DualPortfolioManager:
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.base_btc_weight = self.config.get('btc_weight', 0.7)
        self.base_eth_weight = self.config.get('eth_weight', 0.3)
        ...
        self.eth_performance = AssetPerformance([], [], 0, 0.0, 0.0, 0.0, 0.0)
        ...
        self.logger.info(f"Dual Portfolio Manager: {self.base_btc_weight:.0%} BTC / {self.base_eth_weight:.0%} ETH")
    ...
    def calculate_adaptive_weights(self, current_hour: int) -> Tuple[float, float]:
        ...
        if not self.adaptive_weighting or current_hour < self.sharpe_lookback:
            return self.base_btc_weight, self.base_eth_weight
        
        # Calculate recent Sharpe ratios
        btc_sharpe = self._calculate_recent_sharpe(self.btc_performance.returns[-self.sharpe_lookback:])
        eth_sharpe = self._calculate_recent_sharpe(self.eth_performance.returns[-self.sharpe_lookback:])
        
        if btc_sharpe == 0 and eth_sharpe == 0:
            return self.base_btc_weight, self.base_eth_weight
        
        # Calculate relative performance
        total_sharpe = abs(btc_sharpe) + abs(eth_sharpe)
        if total_sharpe == 0:
            return self.base_btc_weight, self.base_eth_weight
            
        # Adjust weights based on relative Sharpe performance
        if btc_sharpe > eth_sharpe:
            # BTC performing better
            sharpe_diff = (btc_sharpe - eth_sharpe) / total_sharpe
            weight_adjustment = sharpe_diff * self.sharpe_adjustment_factor
            
            new_btc_weight = self.base_btc_weight + weight_adjustment
            new_eth_weight = 1.0 - new_btc_weight
        else:
            # ETH performing better
            sharpe_diff = (eth_sharpe - btc_sharpe) / total_sharpe
            weight_adjustment = sharpe_diff * self.sharpe_adjustment_factor
            
            new_eth_weight = self.base_eth_weight + weight_adjustment
            new_btc_weight = 1.0 - new_eth_weight
        ...
        self.logger.debug(f"Adaptive weights: BTC {new_btc_weight:.1%} (Sharpe: {btc_sharpe:.2f}) / ETH {new_eth_weight:.1%} (Sharpe: {eth_sharpe:.2f})")
        
        return new_btc_weight, new_eth_weight
    ...
    def calculate_portfolio_exposure(self, btc_signal: float, eth_signal: float, current_hour: int) -> Tuple[float, float, Dict]:
        """Calculate final portfolio exposures for both assets"""
        
        # Get adaptive weights
        btc_weight, eth_weight = self.calculate_adaptive_weights(current_hour)
        
        # Apply weights to individual signals
        btc_portfolio_exposure = btc_signal * (btc_weight / self.base_btc_weight)
        eth_portfolio_exposure = eth_signal * (eth_weight / self.base_eth_weight)
        ...
        eth_portfolio_exposure = min(eth_portfolio_exposure, 45.0)   # ETH max exposure
        
        # Portfolio-level risk management
        total_gross_exposure = btc_portfolio_exposure + eth_portfolio_exposure
        ...
        if total_gross_exposure > max_total_exposure:
            ...
            eth_portfolio_exposure *= scale_factor
        
        portfolio_info = {
            "btc_weight": btc_weight,
            "eth_weight": eth_weight,
            "total_exposure": btc_portfolio_exposure + eth_portfolio_exposure,
            "rebalanced": self.should_rebalance(current_hour)
        }
        
        return btc_portfolio_exposure, eth_portfolio_exposure, portfolio_info
    
    def update_performance(self, btc_return: float, eth_return: float, btc_exposure: float, eth_exposure: float):
        ...
        # Update ETH performance  
        self.eth_performance.returns.append(eth_return)
        self.eth_performance.exposures.append(eth_exposure)
        
        # Calculate running statistics
        self._update_asset_stats(self.btc_performance, 'BTC')
        self._update_asset_stats(self.eth_performance, 'ETH')
    ...
    def get_portfolio_summary(self) -> Dict:
        ...
        eth_returns = np.array(self.eth_performance.returns)
        btc_weights = np.array([self.base_btc_weight] * len(btc_returns))
        eth_weights = np.array([self.base_eth_weight] * len(eth_returns))
        
        if len(btc_returns) == 0 or len(eth_returns) == 0:
            return {"error": "Insufficient data for portfolio summary"}
        
        # Portfolio returns (weighted)
        portfolio_returns = btc_returns * btc_weights + eth_returns * eth_weights
        ...
        return {
            ...
            "eth": {
                "total_return": self.eth_performance.total_return * 100,
                "current_dd": self.eth_performance.current_dd,
                "max_dd": self.eth_performance.max_dd,
                "sharpe_ratio": self.eth_performance.sharpe_ratio,
                "trades": self.eth_performance.trades,
                "avg_exposure": np.mean(self.eth_performance.exposures) if self.eth_performance.exposures else 0
            },
            "weights": {
                "btc_base": self.base_btc_weight,
                "eth_base": self.base_eth_weight,
    ...
    def get_current_drawdowns(self) -> Tuple[float, float]:
        ...
        return self.btc_performance.current_dd, self.eth_performance.current_dd