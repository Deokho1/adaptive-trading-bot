"""
ExecutionEngine for processing TradeSignals in dry-run mode.

This module implements the execution layer that consumes TradeSignals
generated by strategies and simulates order execution using public
market data. In dry-run mode, no real orders are placed.
"""
...
class ExecutionEngine:
    """
    Execution layer for TradeSignals.
    
    In dry_run mode, simulates order fills using current ticker prices
    and updates PositionManager without placing real orders.
    """
    ...
    def __init__(
        ...
        if not dry_run:
            self.logger.warning("Real trading mode not implemented yet - forcing dry_run=True")
            self.dry_run = True
        
        self.logger.info(f"ExecutionEngine initialized (dry_run={self.dry_run})")
    
    def __str__(self) -> str:
        """String representation."""
        mode = "DRY_RUN" if self.dry_run else "LIVE"
        positions = len(self.position_manager.get_positions())
        return f"ExecutionEngine(mode={mode}, positions={positions})"
    ...
    def _handle_sell(
        ...
        try:
            ...
            self.logger.info(
                f"📉 SIMULATED SELL: {signal.symbol} "
                f"@ {price:,.0f} KRW, "
