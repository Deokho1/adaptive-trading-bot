"""
SOL Range Mean Reversion Engine  
Specialized for range-bound SOL trading using RSI + Bollinger Bands
Only operates in 'range' market mode, inactive in all other modes
"""
...
class SOLRangeMeanReversionEngine:
    def __init__(self, config: Optional[Dict] = None):
        ...
        # RSI thresholds for range trading
        self.rsi_oversold = self.config.get('rsi_oversold', 30)
        self.rsi_overbought = self.config.get('rsi_overbought', 70)
        
        # Fixed signal strength as requested
        self.signal_strength = 10.0  # Fixed ±10% exposure
        
        self.logger = logging.getLogger(f'{__class__.__name__}')
        self.logger.info(f"SOL Range-Only Mean Reversion Engine initialized: RSI({self.rsi_period}), BB({self.bb_period},{self.bb_std})")
    
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """Calculate SOL-specific range mean reversion indicators"""
        data = df.copy()
        ...
        # Additional technical indicators for context
    ...
    def get_range_mean_reversion_signal(self, row: pd.Series) -> Tuple[str, float]:
    ...
    def generate_signal(self, df: pd.DataFrame, idx: int, market_mode: str, regime_data: Dict, current_dd: float = 0.0) -> Tuple[float, str, Dict]:
        """Generate range-only SOL mean reversion signal"""
        ...
        mr_signal, mr_strength = self.get_range_mean_reversion_signal(row)
        ...
        detail = {
            ...
            "engines": ["sol_range_only_mean_reversion"] if abs(mr_strength) > 0 else ["range_hold"],
            "rsi": row['rsi'],
        ...
        self.logger.debug(f"SOL Range Signal: {signal_type} {total_exposure:.1f}% | Mode: {market_mode} | RSI: {row['rsi']:.1f} | BB: {row['bb_position']:.2f}")
